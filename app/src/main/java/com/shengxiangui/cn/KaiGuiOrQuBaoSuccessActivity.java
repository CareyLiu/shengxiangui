...
public class KaiGuiOrQuBaoSuccessActivity extends Activity {
    ...
    Thread thread;
    @BindView(R.id.iv_kaigui)
    ImageView ivKaigui;
    @BindView(R.id.iv_cunbao)
    ImageView ivCunbao;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layout_kaigui_cunbao);
        ButterKnife.bind(this);
        shouJiHaoOrQuJianMa = getIntent().getStringExtra("shouJiHaoOrQuJianMa");

        if (shouJiHaoOrQuJianMa.equals("qubao")) {
            ivKaigui.setVisibility(View.VISIBLE);
        } else if (shouJiHaoOrQuJianMa.equals("2")) {
            ivCunbao.setVisibility(View.VISIBLE);
        }
    ...
    public static void actionStart(Context context, String shouJiHaoOrQuJianMa) {
        Intent intent = new Intent(context, KaiGuiOrQuBaoSuccessActivity.class);
