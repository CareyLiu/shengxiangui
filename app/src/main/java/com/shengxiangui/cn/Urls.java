package com.shengxiangui.cn;

//网络接口地址
public class Urls {
    public static String key = "20180305124455yu";//全局请求key
 //   public static String SERVER_URL = "https://shop.hljsdkj.com/";//基本地址
//    public static String SERVER_URL = "https://mqrn.hljsdkj.com/";//储物柜
 // public static String SERVER_URL = "http://192.168.2.165:8080/";//大个本地
//    public static String SERVER_URL = "http://192.168.1.110:8080/";//杨哲本地
    public static String SERVER_URL = "https://ggw.hljsdkj.com/";//

    public static String SHENGXIANGUI = SERVER_URL + "cs/app/azp";//生鲜柜相关接口
    public static String SHENGXIANGUI_SHANGPIN = SERVER_URL + "cs/app/azp";//获得商品列表
}
